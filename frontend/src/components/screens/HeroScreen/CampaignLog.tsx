...
const VIEW_META: Record<LogView, { title: string; toggleLabel: string; empty: string }> = {
  experience: { title: 'Campaign Log — Experience', toggleLabel: 'View: Projects', empty: 'No experience on record yet.' },
  projects: { title: 'Campaign Log — Projects', toggleLabel: 'View: Experience', empty: 'No projects on record yet.' },
};
  ...
  return (
    <div className={styles.panel}>
      <div className={styles.header}>
        ...
        <button type="button" className={styles.toggle} onClick={() => setView(view === 'experience' ? 'projects' : 'experience')}>
          {meta.toggleLabel}
        </button>
      </div>
      <div className={styles.entries}>
        {entries.length === 0 && <div className={styles.emptyState}>{meta.empty}</div>}
        {entries.map((row) => {
          ...
          return (
            ...
            >
              ...
              <div className={styles.rail}>
                <StatBar pct={row.progressPct} variant="xp" height={8} outline="idle" />
