...
export function StatBar({ pct, variant, height, outline = 'gold', label, style }: StatBarProps) {
  const trackClass =
    outline === 'idle' ? `${styles.track} ${styles['track--idle']}` : `${styles.track} ${styles['track--outlined']}`;

  return (
    <div className={trackClass} style={{ height, ...style }}>
      <div className={`${styles.fill} ${FILL_CLASS[variant]}`} style={{ width: `${pct}%` }} />
