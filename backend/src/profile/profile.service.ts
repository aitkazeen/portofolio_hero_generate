...
import { ProfileLink, ProfileLinkDocument } from './schemas/profile-link.schema';
...
const PROFILE_NOT_FOUND = 'No profile has been seeded yet — run `npm run seed`.';

@Injectable()
export class ProfileService {
  constructor(
    @InjectModel(Profile.name) private readonly profileModel: Model<ProfileDocument>,
    @InjectModel(ProfileLink.name) private readonly profileLinkModel: Model<ProfileLinkDocument>,
  ...
  async update({ id, ...data }: UpdateProfileInput) {
    const profile = await this.profileModel.findByIdAndUpdate(id, data, { new: true });
    if (!profile) throw new NotFoundException(PROFILE_NOT_FOUND);
    return this.withRelations(profile);
  }
  ...
  private async withRelations(profile: ProfileDocument) {
    const [links, skills, projects, experience] = await Promise.all([
      ...
      this.experienceModel.find({ profileId: profile.id }).sort({ startDate: -1 }),
    ]);
